  ...
  (λ (formula)
    (match formula
      ...
      [`(¬ (∨ ,fs ...))
       `(∧ ,@(for/list ([fi fs])
               (formula->nnf `(¬ ,fi))))]
      [`(,op ,fs ...) `(,op ,@(map formula->nnf fs))]
      [_ formula])))

(define (unnest f)
  (match f
    [`(∨ ,gs ... (∨ ,fs ...) ,hs ...) (unnest `(∨ ,@gs ,@fs ,@hs))]
    [`(∧ ,gs ... (∧ ,fs ...) ,hs ...) (unnest `(∧ ,@gs ,@fs ,@hs))]
...
(define extract-bv-value
  (λ (bv) (string->number (substring (symbol->string bv) 2))))

(define remove-const-bv2fp
  (λ (sexp)
    (match sexp
      [`((_ to_fp ,exp ,sig) (_ ,bv ,width))
       (BitVec->FloatingPoint (mkBV width (extract-bv-value bv)) exp sig)]
    ...
    (match sexp
      [`(fp (_ ,sign ,sig-width) (_ ,exp ,exp-width) (_ ,sig ,sig-width-wo))
       (BitVec->FloatingPoint
        (mkBV (+ (+ sig-width-wo 1) exp-width)
              (+ (arithmetic-shift (extract-bv-value sign)
                                   (+ exp-width sig-width-wo))
                 (+ (arithmetic-shift (extract-bv-value exp) sig-width-wo)
...
(define remove-equalities
  (λ (asserts var-info)
            ...
            (cond
              [(empty? eqs)
               (if hit?
                   cols
                   (cons (list->set vs)
                         cols))] ; if hit ignore else add group to list
              [else
               (let ([eq (car eqs)])
                 (if hit?
                     (try-insert-eqs (cdr eqs)
                                     vs
                                     (cons eq cols)
                                     hit?) ; already hit, continue
                     (if (ormap (λ (v) (set-member? eq v))
          ...
          [`(= ,lhs ,rhs)
           (cond
             [(and (symbol? lhs) (symbol? rhs))
              (try-insert-eqs eqs `(,lhs ,rhs) '() #f)]
             [(symbol? lhs) (try-insert-eqs eqs `(,lhs ,rhs) '() #f)]
             [(symbol? rhs) (try-insert-eqs eqs `(,lhs ,rhs) '() #f)]
             [else eqs])]
          [_ eqs])))
    (define is-eq? (λ (v eqs) (ormap (λ (eq) (set-member? eq v)) eqs)))
    ...
    (let ([eqs (foldl collect-equalities '() asserts)])
      ...
      (if (empty? eqs) asserts (remove-eq-asserts asserts eqs '())))))
